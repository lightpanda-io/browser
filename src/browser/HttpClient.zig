...
pub const Transport = struct {
    ...
    fn processOneMessage(self: *Transport, msg: http.Handles.MultiMessage, transfer: *Transfer) !bool {
        ...
        // a normal end-of-body (the connection closure signals the end of
        // the response per HTTP/1.1 when there is no Content-Length). We
        // must check this before endTransfer, which may reset the easy handle.
