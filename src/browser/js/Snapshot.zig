...
fn attachClass(comptime JsApi: type, isolate: *v8.Isolate, template: *v8.FunctionTemplate) void {
    ...
    inline for (declarations) |d| {
        ...
        switch (definition) {
            ...
            bridge.Indexed => {
                ...
                v8.v8__ObjectTemplate__SetIndexedHandler(target, &configuration);
            },
            bridge.NamedIndexed => {
                const instance_template = v8.v8__FunctionTemplate__InstanceTemplate(template);
                ...
                v8.v8__ObjectTemplate__SetNamedHandler(instance_template, &configuration);
            },
            bridge.Iterator => {
                const function_template = @constCast(v8.v8__FunctionTemplate__New__DEFAULT2(isolate, value.func).?);
    ...
    if (@hasDecl(JsApi.Meta, "htmldda")) {
        const instance_template = v8.v8__FunctionTemplate__InstanceTemplate(template);
        v8.v8__ObjectTemplate__MarkAsUndetectable(instance_template);
        v8.v8__ObjectTemplate__SetCallAsFunctionHandler(instance_template, JsApi.Meta.callable.func);
    }
}
