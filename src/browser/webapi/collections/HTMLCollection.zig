...
pub const JsApi = struct {
    ...
    // Deliberately NOT ReadOnly: v8 consults the query when a [[Set]] walks
    // the prototype chain, and a read-only property on the prototype would
    // block the expando that the spec's ignore-named-props rule requires
    // (HTMLCollection-as-prototype.html). Writability as seen by
    // getOwnPropertyDescriptor comes from describeByName, and direct
    // assignments still fail through defineByName.
