...
pub const Connection = struct {
    ...
    pub const Request = struct {
        ...
        path: []const u8,
        // what followed the '?' in the request-target, "" when there was none
        query: []const u8,
        keepalive: bool,
        body: []const u8,
    ...
    pub const State = union(enum) {
        ...
        fn parseHeader(self: *State, arena: Allocator, data: []u8) !bool {
            ...
            const method, const path, const query, const keepalive, const line_1_end = try parseRequestLine(header);
        ...
        fn parseRequestLine(header: []const u8) !struct { Method, []const u8, []const u8, bool, usize } {
            ...
            // Only the origin-form request-target is accepted. Routing ignores
            // the query string, as Chrome does.
            ...
            return .{ method, path, query, keepalive, l1 };
        }
    };
...
const target_activated_response = staticResponse(.{ .status = "200 OK", .body = "Target activated", .content_type = "text/plain; charset=UTF-8" });
const target_closing_response = staticResponse(.{ .status = "200 OK", .body = "Target is closing", .content_type = "text/plain; charset=UTF-8" });
...
const Route = struct {
    ...
    // match against the normalized path: exact, or exact plus any
    // "/{suffix}" for the routes Chrome addresses by target id
    path: []const u8,
...
const routes = [_]Route{
    ...
    // PUT is what Chrome requires since 66; GET is kept for older clients
    .{ .method = .GET, .path = "/json/new", .gate = .cdp, .handler = serveJSONNew },
    .{ .method = .PUT, .path = "/json/new", .gate = .cdp, .handler = serveJSONNew },
    .{ .method = .GET, .path = "/json/activate", .match = .prefix, .gate = .cdp, .handler = serveJSONActivate },
    .{ .method = .GET, .path = "/json/close", .match = .prefix, .gate = .cdp, .handler = serveJSONClose },
...
fn dispatch(server: *Server, comptime table: []const Route, conn: *Connection, req: *Connection.Request, path: []const u8) !Served {
    ...
    inline for (table) |route| {
        const matched = std.mem.eql(u8, route.path, path) or
            (route.match == .prefix and std.mem.startsWith(u8, path, route.path ++ "/"));
        if (matched and gateOpen(server, route.gate)) {
...
fn serveJSONVersion(server: *Server, conn: *Connection, req: *Connection.Request) !Served {
    return serveHTTPResponse(server, conn, req, .{ .static = server.http_responses.version });
}

// GET /json/list or GET /json (cdp)
fn serveJSONList(server: *Server, conn: *Connection, req: *Connection.Request) !Served {
    return serveHTTPResponse(server, conn, req, .{ .static = server.http_responses.list });
}

// GET or PUT /json/new[?url] (cdp)
fn serveJSONNew(server: *Server, conn: *Connection, req: *Connection.Request) !Served {
    if (req.query.len > 0) {
        log.warn(.not_implemented, "json new navigation", .{ .url = req.query });
    }
    return serveHTTPResponse(server, conn, req, .{ .static = server.http_responses.new });
}

// GET /json/activate/{id} (cdp)
fn serveJSONActivate(server: *Server, conn: *Connection, req: *Connection.Request) !Served {
    return serveHTTPResponse(server, conn, req, .{ .static = target_activated_response });
}

// GET /json/close/{id} (cdp)
fn serveJSONClose(server: *Server, conn: *Connection, req: *Connection.Request) !Served {
    log.warn(.not_implemented, "json close target", .{});
    return serveHTTPResponse(server, conn, req, .{ .static = target_closing_response });
}
...
// The /json discovery responses, built once the listener knows its port.
pub const Responses = struct {
    version: []const u8 = "",
    list: []const u8 = "",
    new: []const u8 = "",

    pub fn init(app: *const App, port: u16) !Responses {
        const allocator = app.allocator;
        const host = app.config.advertiseHost();

        const version = try buildJSONVersionResponse(app, port);
        errdefer allocator.free(version);
        const list = try buildJSONResponse(allocator, "[ " ++ target_json_format ++ " ]", .{ host, port });
        errdefer allocator.free(list);
        const new = try buildJSONResponse(allocator, target_json_format, .{ host, port });
        errdefer allocator.free(new);

        return .{ .version = version, .list = list, .new = new };
    }

    pub fn deinit(self: *const Responses, allocator: Allocator) void {
...
fn buildJSONResponse(allocator: Allocator, comptime body_format: []const u8, args: anytype) ![]const u8 {
    ...
    return std.fmt.allocPrint(allocator, response_format, .{body_len} ++ args);
}
...
pub fn buildJSONVersionResponse(app: *const App, port: u16) ![]const u8 {
    ...
    const body_format =
        ...
        "\"webSocketDebuggerUrl\": \"ws://{s}:{d}/\"" ++
        "}}";
    return buildJSONResponse(app.allocator, body_format, .{ host, port });
}
