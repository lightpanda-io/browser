...
pub const Input = struct {
    ...
    pub fn pushPointer(self: *Input, allocator: Allocator, x: i32, y: i32, button: Button, pressed: bool) !void {
        try self.push(allocator, .{ .pointer = .{ .x = x, .y = y, .button = button, .pressed = pressed } });
    }

    pub fn pushWheel(self: *Input, allocator: Allocator, delta_x: i32, delta_y: i32) !void {
        try self.push(allocator, .{ .wheel = .{ .delta_x = delta_x, .delta_y = delta_y } });
    }
...
test "input queue preserves event order" {
    ...
    try input.pushPointer(std.testing.allocator, 10, 12, .left, true);
    try input.pushWheel(std.testing.allocator, 0, -1);
