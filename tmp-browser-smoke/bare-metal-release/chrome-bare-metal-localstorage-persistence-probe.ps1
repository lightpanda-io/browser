...
$profileRoot = Join-Path $root "profile-localstorage-restart"
...
$browserOneOut = Join-Path $root "chrome-bare-metal-localstorage-restart.run1.browser.stdout.txt"
$browserOneErr = Join-Path $root "chrome-bare-metal-localstorage-restart.run1.browser.stderr.txt"
$browserTwoOut = Join-Path $root "chrome-bare-metal-localstorage-restart.run2.browser.stdout.txt"
$browserTwoErr = Join-Path $root "chrome-bare-metal-localstorage-restart.run2.browser.stderr.txt"
$serverOut = Join-Path $root "chrome-bare-metal-localstorage-restart.server.stdout.txt"
$serverErr = Join-Path $root "chrome-bare-metal-localstorage-restart.server.stderr.txt"
...
try {
  if (-not (Test-Path $manifestPath) -or -not (Test-Path $bootBinary) -or -not (Test-Path $archivePath)) {
    & $packageScript -PackageRoot $packageRoot -Url "https://example.com/" | Tee-Object -FilePath (Join-Path $root "chrome-bare-metal-localstorage-restart.package.stdout.txt") | ConvertFrom-Json | Out-Null
  }
  ...
  $titles.restart = Wait-TabTitle $browserTwo.Id "Local Storage Echo ok" 40
