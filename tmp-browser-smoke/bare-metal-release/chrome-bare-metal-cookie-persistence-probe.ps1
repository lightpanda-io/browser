...
$port = 8194
$browserOneOut = Join-Path $root "chrome-bare-metal-cookie-restart.run1.browser.stdout.txt"
$browserOneErr = Join-Path $root "chrome-bare-metal-cookie-restart.run1.browser.stderr.txt"
$browserTwoOut = Join-Path $root "chrome-bare-metal-cookie-restart.run2.browser.stdout.txt"
$browserTwoErr = Join-Path $root "chrome-bare-metal-cookie-restart.run2.browser.stderr.txt"
$serverOut = Join-Path $root "chrome-bare-metal-cookie-restart.server.stdout.txt"
$serverErr = Join-Path $root "chrome-bare-metal-cookie-restart.server.stderr.txt"
...
try {
  if (-not (Test-Path $manifestPath) -or -not (Test-Path $bootBinary) -or -not (Test-Path $archivePath)) {
    & $packageScript -PackageRoot $packageRoot -Url "https://example.com/" | Tee-Object -FilePath (Join-Path $root "chrome-bare-metal-cookie-restart.package.stdout.txt") | ConvertFrom-Json | Out-Null
  }
