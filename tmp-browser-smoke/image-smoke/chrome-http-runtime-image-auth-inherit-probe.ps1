...
for ($i = 0; $i -lt 40; $i++) {
  ...
  try {
    $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/auth-inherit-page.html" -TimeoutSec 2
    if ($resp.StatusCode -eq 200) { $ready = $true; break }
  } catch {}
}
if (-not $ready) {
  throw "localhost image inherited auth server did not become ready"
}
