...
$outPng = Join-Path $root "checkbox-pair-submit.png"
$browserOut = Join-Path $root "checkbox-pair-submit.browser.stdout.txt"
$browserErr = Join-Path $root "checkbox-pair-submit.browser.stderr.txt"
$serverOut = Join-Path $root "checkbox-pair-submit.server.stdout.txt"
$serverErr = Join-Path $root "checkbox-pair-submit.server.stderr.txt"
...
try {
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/checkbox-pair-submit.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "inline checkbox pair submit probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-checkbox-pair-submit"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","http://127.0.0.1:$port/checkbox-pair-submit.html","--window_width","760","--window_height","560","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
    if ((Test-Path $outPng) -and ((Get-Item $outPng).Length -gt 0)) { $pngReady = $true; break }
  }
  if (-not $pngReady) { throw "inline checkbox pair submit screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "inline checkbox pair submit window handle not found" }
  ...
  if ($null -eq $checkboxOne.min_y) { throw "inline checkbox pair submit probe did not isolate the first checkbox control" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterCheckboxOne -like "Dense Checkbox Submit one true*") {
      $checkboxOneWorked = $true
      break
    }
  }
  if (-not $checkboxOneWorked) { throw "inline checkbox pair submit probe first checkbox did not toggle on click" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterCheckboxTwo -like "Dense Checkbox Submit two true*") {
      $checkboxTwoWorked = $true
      break
    }
  }
  if (-not $checkboxTwoWorked) { throw "inline checkbox pair submit probe second checkbox did not toggle on space after tab" }

  Send-SmokeTab
  Start-Sleep -Milliseconds 120
  Send-SmokeSpace
  $titleAfterSubmit = $titleAfterCheckboxTwo
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 200
    $titleAfterSubmit = Get-SmokeWindowTitle $hwnd
    if ($titleAfterSubmit -like "Inline Checkbox Pair Submitted*") {
      $submitWorked = $true
      break
    }
  }
  if (-not $submitWorked -and (Test-Path $serverErr)) {
    ...
    $serverSawSubmit = $serverLog -match 'GET /submitted-checkbox-pair\.html(\?| )'
    if ($serverSawSubmit) {
      $submitWorked = $true
    }
  }
