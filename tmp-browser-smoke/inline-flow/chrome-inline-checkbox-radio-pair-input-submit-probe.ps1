...
$outPng = Join-Path $root "checkbox-radio-pair-input-submit.png"
$browserOut = Join-Path $root "checkbox-radio-pair-input-submit.browser.stdout.txt"
$browserErr = Join-Path $root "checkbox-radio-pair-input-submit.browser.stderr.txt"
$serverOut = Join-Path $root "checkbox-radio-pair-input-submit.server.stdout.txt"
$serverErr = Join-Path $root "checkbox-radio-pair-input-submit.server.stderr.txt"
...
try {
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/checkbox-radio-pair-input-submit.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "inline checkbox radio pair input submit probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-checkbox-radio-pair-input-submit"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","http://127.0.0.1:$port/checkbox-radio-pair-input-submit.html","--window_width","760","--window_height","560","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
    if ((Test-Path $outPng) -and ((Get-Item $outPng).Length -gt 0)) { $pngReady = $true; break }
  }
  if (-not $pngReady) { throw "inline checkbox radio pair input submit screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "inline checkbox radio pair input submit window handle not found" }
  ...
  if ($null -eq $checkboxOne.min_y) { throw "inline checkbox radio pair input submit probe did not isolate the first checkbox control" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterCheckboxOne -like "Dense Multi checkbox one true*") {
      $checkboxOneWorked = $true
      break
    }
  }
  if (-not $checkboxOneWorked) { throw "inline checkbox radio pair input submit probe first checkbox did not toggle on click" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterCheckboxTwo -like "Dense Multi checkbox two true*") {
      $checkboxTwoWorked = $true
      break
    }
  }
  if (-not $checkboxTwoWorked) { throw "inline checkbox radio pair input submit probe second checkbox did not toggle on space after tab" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterRadioOne -like "Dense Multi radio one true*") {
      $radioOneWorked = $true
      break
    }
  }
  if (-not $radioOneWorked) { throw "inline checkbox radio pair input submit probe first radio did not select on space after tab" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterRadioTwo -like "Dense Multi radio two true*") {
      $radioTwoWorked = $true
      break
    }
  }
  if (-not $radioTwoWorked) { throw "inline checkbox radio pair input submit probe second radio did not select on space after tab" }

  Send-SmokeTab
  Start-Sleep -Milliseconds 120
  Send-SmokeText "QZ"
  $titleAfterInput = $titleAfterRadioTwo
  for ($i = 0; $i -lt 40; $i++) {
    Start-Sleep -Milliseconds 150
    $titleAfterInput = Get-SmokeWindowTitle $hwnd
    if ($titleAfterInput -like "Dense Multi entry QZ*") {
      $inputWorked = $true
      break
    }
  }
  if (-not $inputWorked) { throw "inline checkbox radio pair input submit probe input did not update after typing" }
  ...
  for ($i = 0; $i -lt 60; $i++) {
    ...
    if ($titleAfterSubmit -like "Inline Checkbox Radio Pair Input Submitted*") {
      $submitWorked = $true
      break
    }
  }
  if (-not $submitWorked -and (Test-Path $serverErr)) {
    ...
    $serverSawSubmit = $serverLog -match 'GET /submitted-checkbox-radio-pair-input\.html(\?| )'
    if ($serverSawSubmit) {
      $submitWorked = $true
    }
  }
