...
$outPng = Join-Path $root "checkbox-radio-pair-two-input-seven-link-four.png"
$browserOut = Join-Path $root "checkbox-radio-pair-two-input-seven-link-four.browser.stdout.txt"
$browserErr = Join-Path $root "checkbox-radio-pair-two-input-seven-link-four.browser.stderr.txt"
$serverOut = Join-Path $root "checkbox-radio-pair-two-input-seven-link-four.server.stdout.txt"
$serverErr = Join-Path $root "checkbox-radio-pair-two-input-seven-link-four.server.stderr.txt"
...
try {
  ...
  $profileRoot = Join-Path $root "profile-inline-checkbox-radio-pair-two-input-seven-link-four"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  if (-not $inputTwoWorked) { throw "inline checkbox radio pair two input seven-link fourth probe second input did not update after typing" }
  ...
  for ($i = 0; $i -lt 60; $i++) {
    ...
    if ($titleAfterLink -like "Inline Flow Target Four*") { $linkWorked = $true; break }
  }
  if (-not $linkWorked -and (Test-Path $serverErr)) {
    $serverLog = Get-Content $serverErr -Raw
    $serverSawNext = $serverLog -match 'GET /next-four\.html(\?| )'
