...
$outPng = Join-Path $root "checkbox-radio-pair-two-input-six-link-submit.png"
$browserOut = Join-Path $root "checkbox-radio-pair-two-input-six-link-submit.browser.stdout.txt"
$browserErr = Join-Path $root "checkbox-radio-pair-two-input-six-link-submit.browser.stderr.txt"
$serverOut = Join-Path $root "checkbox-radio-pair-two-input-six-link-submit.server.stdout.txt"
$serverErr = Join-Path $root "checkbox-radio-pair-two-input-six-link-submit.server.stderr.txt"
...
try {
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/checkbox-radio-pair-two-input-six-link-submit.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "inline checkbox radio pair two input six-link submit probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-checkbox-radio-pair-two-input-six-link-submit"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","http://127.0.0.1:$port/checkbox-radio-pair-two-input-six-link-submit.html","--window_width","760","--window_height","560","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
    if ((Test-Path $outPng) -and ((Get-Item $outPng).Length -gt 0)) { $pngReady = $true; break }
  }
  if (-not $pngReady) { throw "inline checkbox radio pair two input six-link submit screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "inline checkbox radio pair two input six-link submit window handle not found" }
  ...
  if ($null -eq $checkboxOne.min_y) { throw "inline checkbox radio pair two input six-link submit probe did not isolate the first checkbox control" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterCheckboxOne -like "Dense Six Link checkbox one true*") { $checkboxOneWorked = $true; break }
  }
  if (-not $checkboxOneWorked) { throw "inline checkbox radio pair two input six-link submit probe first checkbox did not toggle on click" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterCheckboxTwo -like "Dense Six Link checkbox two true*") { $checkboxTwoWorked = $true; break }
  }
  if (-not $checkboxTwoWorked) { throw "inline checkbox radio pair two input six-link submit probe second checkbox did not toggle on space after tab" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterRadioOne -like "Dense Six Link radio one true*") { $radioOneWorked = $true; break }
  }
  if (-not $radioOneWorked) { throw "inline checkbox radio pair two input six-link submit probe first radio did not select on space after tab" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterRadioTwo -like "Dense Six Link radio two true*") { $radioTwoWorked = $true; break }
  }
  if (-not $radioTwoWorked) { throw "inline checkbox radio pair two input six-link submit probe second radio did not select on space after tab" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterInputOne -like "Dense Six Link entry one QZ*") { $inputOneWorked = $true; break }
  }
  if (-not $inputOneWorked) { throw "inline checkbox radio pair two input six-link submit probe first input did not update after typing" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterInputTwo -like "Dense Six Link entry two LM*") { $inputTwoWorked = $true; break }
  }
  if (-not $inputTwoWorked) { throw "inline checkbox radio pair two input six-link submit probe second input did not update after typing" }
  ...
  for ($i = 0; $i -lt 60; $i++) {
    ...
    if ($titleAfterSubmit -like "Inline Checkbox Radio Pair Two Input Six Link Submitted*") { $submitWorked = $true; break }
  }
  if (-not $submitWorked -and (Test-Path $serverErr)) {
    $serverLog = Get-Content $serverErr -Raw
    $serverSawSubmit = $serverLog -match 'GET /submitted-checkbox-radio-pair-two-input-six-link\.html(\?| )'
    if ($serverSawSubmit) { $submitWorked = $true }
  }
  if (-not $submitWorked) { throw "inline checkbox radio pair two input six-link submit probe did not reach the submitted page" }
}
catch {
