...
$outPng = Join-Path $root "checkbox-radio-input-submit.png"
$browserOut = Join-Path $root "checkbox-radio-input-submit.browser.stdout.txt"
$browserErr = Join-Path $root "checkbox-radio-input-submit.browser.stderr.txt"
$serverOut = Join-Path $root "checkbox-radio-input-submit.server.stdout.txt"
$serverErr = Join-Path $root "checkbox-radio-input-submit.server.stderr.txt"
...
try {
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/checkbox-radio-input-submit.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "inline checkbox radio input submit probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-checkbox-radio-input-submit"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","http://127.0.0.1:$port/checkbox-radio-input-submit.html","--window_width","760","--window_height","560","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
    if ((Test-Path $outPng) -and ((Get-Item $outPng).Length -gt 0)) { $pngReady = $true; break }
  }
  if (-not $pngReady) { throw "inline checkbox radio input submit screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "inline checkbox radio input submit window handle not found" }
  ...
  if ($null -eq $checkbox.min_y) { throw "inline checkbox radio input submit probe did not isolate the checkbox control" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterCheckbox -like "Dense Combo Input checkbox true*") {
      $checkboxWorked = $true
      break
    }
  }
  if (-not $checkboxWorked) { throw "inline checkbox radio input submit probe checkbox did not toggle on click" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterRadio -like "Dense Combo Input radio true*") {
      $radioWorked = $true
      break
    }
  }
  if (-not $radioWorked) { throw "inline checkbox radio input submit probe radio did not select on space after tab" }

  Send-SmokeTab
  Start-Sleep -Milliseconds 120
  Send-SmokeText "QZ"
  $titleAfterInput = $titleAfterRadio
  for ($i = 0; $i -lt 40; $i++) {
    Start-Sleep -Milliseconds 150
    $titleAfterInput = Get-SmokeWindowTitle $hwnd
    if ($titleAfterInput -like "Dense Combo Input entry QZ*") {
      $inputWorked = $true
      break
    }
  }
  if (-not $inputWorked) { throw "inline checkbox radio input submit probe input did not update after typing" }
  ...
  for ($i = 0; $i -lt 60; $i++) {
    ...
    if ($titleAfterSubmit -like "Inline Checkbox Radio Input Submitted*") {
      $submitWorked = $true
      break
    }
  }
  if (-not $submitWorked -and (Test-Path $serverErr)) {
    ...
    $serverSawSubmit = $serverLog -match 'GET /submitted-checkbox-radio-input\.html(\?| )'
    if ($serverSawSubmit) {
      $submitWorked = $true
    }
  }
