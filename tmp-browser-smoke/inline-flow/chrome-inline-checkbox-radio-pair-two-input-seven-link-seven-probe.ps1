...
$outPng = Join-Path $root "checkbox-radio-pair-two-input-seven-link-seven.png"
$browserOut = Join-Path $root "checkbox-radio-pair-two-input-seven-link-seven.browser.stdout.txt"
$browserErr = Join-Path $root "checkbox-radio-pair-two-input-seven-link-seven.browser.stderr.txt"
$serverOut = Join-Path $root "checkbox-radio-pair-two-input-seven-link-seven.server.stdout.txt"
$serverErr = Join-Path $root "checkbox-radio-pair-two-input-seven-link-seven.server.stderr.txt"
...
try {
  ...
  if (-not $ready) { throw "inline checkbox radio pair two input seven-link seventh probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-checkbox-radio-pair-two-input-seven-link-seven"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  if (-not $pngReady) { throw "inline checkbox radio pair two input seven-link third screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "inline checkbox radio pair two input seven-link third window handle not found" }
  ...
  if ($null -eq $checkboxOne.min_y) { throw "inline checkbox radio pair two input seven-link seventh probe did not isolate the first checkbox control" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterCheckboxOne -like "Dense Seven Link checkbox one true*") {
      $checkboxOneWorked = $true
      break
    }
  }
  if (-not $checkboxOneWorked) { throw "inline checkbox radio pair two input seven-link seventh probe first checkbox did not toggle on click" }
  ...
  if (-not $checkboxTwoWorked) { throw "inline checkbox radio pair two input seven-link seventh probe second checkbox did not toggle on space after tab" }
  ...
  if (-not $radioOneWorked) { throw "inline checkbox radio pair two input seven-link seventh probe first radio did not select on space after tab" }
  ...
  if (-not $radioTwoWorked) { throw "inline checkbox radio pair two input seven-link seventh probe second radio did not select on space after tab" }
  ...
  if (-not $inputOneWorked) { throw "inline checkbox radio pair two input seven-link seventh probe first input did not update after typing" }
  ...
  if (-not $inputTwoWorked) { throw "inline checkbox radio pair two input seven-link seventh probe second input did not update after typing" }
  ...
  for ($i = 0; $i -lt 60; $i++) {
    ...
    if ($titleAfterLink -like "Inline Flow Target Seven*") { $linkWorked = $true; break }
  }
  if (-not $linkWorked -and (Test-Path $serverErr)) {
    $serverLog = Get-Content $serverErr -Raw
    $serverSawNext = $serverLog -match 'GET /next-seven\.html(\?| )'
