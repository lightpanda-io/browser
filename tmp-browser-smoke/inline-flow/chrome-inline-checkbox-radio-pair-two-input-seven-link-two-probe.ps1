...
$outPng = Join-Path $root "checkbox-radio-pair-two-input-seven-link-two.png"
$browserOut = Join-Path $root "checkbox-radio-pair-two-input-seven-link-two.browser.stdout.txt"
$browserErr = Join-Path $root "checkbox-radio-pair-two-input-seven-link-two.browser.stderr.txt"
$serverOut = Join-Path $root "checkbox-radio-pair-two-input-seven-link-two.server.stdout.txt"
$serverErr = Join-Path $root "checkbox-radio-pair-two-input-seven-link-two.server.stderr.txt"
...
try {
  ...
  if (-not $ready) { throw "inline checkbox radio pair two input seven-link second probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-checkbox-radio-pair-two-input-seven-link-two"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  if (-not $pngReady) { throw "inline checkbox radio pair two input seven-link second screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "inline checkbox radio pair two input seven-link second window handle not found" }
  ...
  if ($null -eq $checkboxOne.min_y) { throw "inline checkbox radio pair two input seven-link second probe did not isolate the first checkbox control" }
  ...
  if (-not $checkboxOneWorked) { throw "inline checkbox radio pair two input seven-link second probe first checkbox did not toggle on click" }
  ...
  if (-not $checkboxTwoWorked) { throw "inline checkbox radio pair two input seven-link second probe second checkbox did not toggle on space after tab" }
  ...
  if (-not $radioOneWorked) { throw "inline checkbox radio pair two input seven-link second probe first radio did not select on space after tab" }
  ...
  if (-not $radioTwoWorked) { throw "inline checkbox radio pair two input seven-link second probe second radio did not select on space after tab" }
  ...
  if (-not $inputOneWorked) { throw "inline checkbox radio pair two input seven-link second probe first input did not update after typing" }
  ...
  if (-not $inputTwoWorked) { throw "inline checkbox radio pair two input seven-link second probe second input did not update after typing" }
