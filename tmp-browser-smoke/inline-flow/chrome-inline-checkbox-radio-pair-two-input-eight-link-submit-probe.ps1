...
$outPng = Join-Path $root "checkbox-radio-pair-two-input-eight-link-submit.png"
$browserOut = Join-Path $root "checkbox-radio-pair-two-input-eight-link-submit.browser.stdout.txt"
$browserErr = Join-Path $root "checkbox-radio-pair-two-input-eight-link-submit.browser.stderr.txt"
$serverOut = Join-Path $root "checkbox-radio-pair-two-input-eight-link-submit.server.stdout.txt"
$serverErr = Join-Path $root "checkbox-radio-pair-two-input-eight-link-submit.server.stderr.txt"
...
try {
  ...
  if (-not $ready) { throw "inline checkbox radio pair two input eight-link submit probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-checkbox-radio-pair-two-input-eight-link-submit"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  if (-not $pngReady) { throw "inline checkbox radio pair two input eight-link submit screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "inline checkbox radio pair two input eight-link submit window handle not found" }
  ...
  if ($null -eq $checkboxOne.min_y) { throw "inline checkbox radio pair two input eight-link submit probe did not isolate the first checkbox control" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterCheckboxOne -like "Dense Eight Link checkbox one true*") { $checkboxOneWorked = $true; break }
  }
  if (-not $checkboxOneWorked) { throw "inline checkbox radio pair two input eight-link submit probe first checkbox did not toggle on click" }
  ...
  if (-not $checkboxTwoWorked) { throw "inline checkbox radio pair two input eight-link submit probe second checkbox did not toggle on space after tab" }
  ...
  if (-not $radioOneWorked) { throw "inline checkbox radio pair two input eight-link submit probe first radio did not select on space after tab" }
  ...
  if (-not $radioTwoWorked) { throw "inline checkbox radio pair two input eight-link submit probe second radio did not select on space after tab" }
  ...
  if (-not $inputOneWorked) { throw "inline checkbox radio pair two input eight-link submit probe first input did not update after typing" }
  ...
  if (-not $inputTwoWorked) { throw "inline checkbox radio pair two input eight-link submit probe second input did not update after typing" }
  ...
  for ($i = 0; $i -lt 60; $i++) {
    ...
    if ($titleAfterSubmit -like "Inline Checkbox Radio Pair Two Input Eight Link Submitted*") { $submitWorked = $true; break }
  }
  if (-not $submitWorked -and (Test-Path $serverErr)) {
    $serverLog = Get-Content $serverErr -Raw
    $serverSawSubmit = $serverLog -match 'GET /submitted-checkbox-radio-pair-two-input-eight-link\.html(\?| )'
    if ($serverSawSubmit) { $submitWorked = $true }
  }
  if (-not $submitWorked) { throw "inline checkbox radio pair two input eight-link submit probe did not reach the submitted page" }
}
catch {
