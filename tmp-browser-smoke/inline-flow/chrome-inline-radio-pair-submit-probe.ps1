...
$outPng = Join-Path $root "radio-pair-submit.png"
$browserOut = Join-Path $root "radio-pair-submit.browser.stdout.txt"
$browserErr = Join-Path $root "radio-pair-submit.browser.stderr.txt"
$serverOut = Join-Path $root "radio-pair-submit.server.stdout.txt"
$serverErr = Join-Path $root "radio-pair-submit.server.stderr.txt"
...
try {
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    try {
      $resp = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:$port/radio-pair-submit.html" -TimeoutSec 2
      if ($resp.StatusCode -eq 200) { $ready = $true; break }
    } catch {}
  }
  if (-not $ready) { throw "inline radio pair submit probe server did not become ready" }

  $profileRoot = Join-Path $root "profile-inline-radio-pair-submit"
...
"@ | Set-Content -Path (Join-Path $appDataRoot "browse-settings-v1.txt") -NoNewline
  ...
  $browser = Start-Process -FilePath $browserExe -ArgumentList "browse","http://127.0.0.1:$port/radio-pair-submit.html","--window_width","760","--window_height","560","--screenshot_png",$outPng -WorkingDirectory $repo -PassThru -RedirectStandardOutput $browserOut -RedirectStandardError $browserErr
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 250
    if ((Test-Path $outPng) -and ((Get-Item $outPng).Length -gt 0)) { $pngReady = $true; break }
  }
  if (-not $pngReady) { throw "inline radio pair submit screenshot did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "inline radio pair submit window handle not found" }
  ...
  if ($null -eq $radioOne.min_y) { throw "inline radio pair submit probe did not isolate the first radio control" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterRadioOne -like "Dense Radio Submit one true*") {
      $radioOneWorked = $true
      break
    }
  }
  if (-not $radioOneWorked) { throw "inline radio pair submit probe first radio did not select on click" }
  ...
  for ($i = 0; $i -lt 40; $i++) {
    ...
    if ($titleAfterRadioTwo -like "Dense Radio Submit two true*") {
      $radioTwoWorked = $true
      break
    }
  }
  if (-not $radioTwoWorked) { throw "inline radio pair submit probe second radio did not select on space after tab" }

  Send-SmokeTab
  Start-Sleep -Milliseconds 120
  Send-SmokeSpace
  $titleAfterSubmit = $titleAfterRadioTwo
  for ($i = 0; $i -lt 60; $i++) {
    Start-Sleep -Milliseconds 200
    $titleAfterSubmit = Get-SmokeWindowTitle $hwnd
    if ($titleAfterSubmit -like "Inline Radio Pair Submitted*") {
      $submitWorked = $true
      break
    }
  }
  if (-not $submitWorked -and (Test-Path $serverErr)) {
    ...
    $serverSawSubmit = $serverLog -match 'GET /submitted-radio-pair\.html(\?| )'
    if ($serverSawSubmit) {
      $submitWorked = $true
    }
  }
