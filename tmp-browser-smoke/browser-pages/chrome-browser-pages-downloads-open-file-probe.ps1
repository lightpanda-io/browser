...
$profileRoot = Join-Path $Root "profile-download-open-file"
$app = Reset-BrowserPagesProfile $profileRoot
$port = 8201
$browserOut = Join-Path $Root "chrome-browser-pages-downloads-open-file.browser.stdout.txt"
$browserErr = Join-Path $Root "chrome-browser-pages-downloads-open-file.browser.stderr.txt"
$serverOut = Join-Path $Root "chrome-browser-pages-downloads-open-file.server.stdout.txt"
$serverErr = Join-Path $Root "chrome-browser-pages-downloads-open-file.server.stderr.txt"
$shellLog = Join-Path $Root "chrome-browser-pages-downloads-open-file.shell.log"
...
try {
  ...
  if (-not $ready) { throw "downloads open-file server did not become ready" }
  ...
  if ($hwnd -eq [IntPtr]::Zero) { throw "downloads open-file window handle not found" }
  ...
  Invoke-BrowserPagesAddressCommit $hwnd "browser://downloads/open-file/0"
  ...
  $actionWorked = $logLines.Count -ge 1 -and $logLines[-1] -eq "open-file`t$seedDownloadPath"
  if (-not $actionWorked) { throw "open-file shell action did not log the expected path" }
  if (-not $pageStayed) { throw "downloads page did not remain open after open-file action" }
} catch {
