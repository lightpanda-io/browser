...
//! copy shares those, so all copies are read.
    ...
    fn lessThan(_: void, a: Loc, b: Loc) bool {
        return switch (std.mem.order(u8, a.archive, b.archive)) {
            ...
            .eq => switch (std.mem.order(u8, a.file, b.file)) {
                .lt => true,
                .gt => false,
                .eq => std.mem.order(u8, a.section, b.section) == .lt,
            },
        };
    }
};
...
const Stats = std.StringArrayHashMapUnmanaged(usize);
...
pub fn main(init: std.process.Init) !void {
    ...
    while (args.next()) |arg| {
        if (std.mem.eql(u8, arg, "--v8")) {
            ...
            stats_path = args.next() orelse return error.Usage;
    ...
    if (zig_cache) |dir_path| try findZigCacheArchives(gpa, io, dir_path, &inputs);

    var loc_of: LocOf = .empty;
    for (inputs.keys()) |path| try index(gpa, io, path, &loc_of);

    const v8_archive = if (v8) |v| std.fs.path.basename(v.archive) else null;
    var stats: Stats = .empty;
    var v8_syms: std.StringArrayHashMapUnmanaged(void) = .empty;
    var out: std.ArrayList(u8) = .empty;
    try out.appendSlice(gpa, "/* Generated by orderfile/gen_order.zig, see orderfile/README.md. */\n");
    try out.appendSlice(gpa, "SECTIONS {\n  .text.hot : {\n");
    try emit(gpa, io, hot_text, ".text", &loc_of, v8_archive, &v8_syms, &stats, &out);
    try out.appendSlice(gpa, "\n  }\n} INSERT BEFORE .text;\n");
    try out.appendSlice(gpa, "SECTIONS {\n  .rodata.hot : {\n");
    try emit(gpa, io, hot_rodata, ".rodata", &loc_of, v8_archive, &v8_syms, &stats, &out);
    try out.appendSlice(gpa, "\n  }\n} INSERT BEFORE .rodata;\n");
    try cwd.writeFile(io, .{ .sub_path = out_path, .data = out.items });

    if (v8) |v| {
        var list: std.ArrayList(u8) = .empty;
        for (v8_syms.keys()) |name| {
    ...
    const keys = try gpa.dupe([]const u8, stats.keys());
    std.mem.sort([]const u8, keys, {}, stringLessThan);
    var report: std.ArrayList(u8) = .empty;
    for (keys) |key| {
        try report.print(gpa, "{s}: {d}\n", .{ key, stats.get(key).? });
    }
    if (stats_path) |path| {
        try cwd.writeFile(io, .{ .sub_path = path, .data = report.items });
    }
    var buf: [1024]u8 = undefined;
    var stdout = std.Io.File.stdout().writer(io, &buf);
    try stdout.interface.writeAll(report.items);
...
fn findZigCacheArchives(gpa: Allocator, io: std.Io, dir_path: []const u8, inputs: *std.StringArrayHashMapUnmanaged(void)) !void {
    const cwd = std.Io.Dir.cwd();
    ...
    while (try it.next(io)) |entry| {
        ...
        for (zig_cache_archives) |name| {
            ...
            try inputs.put(gpa, path, {});
        }
    }
}

/// Records where every .text* / .rodata* symbol of `path` (an object or an
/// archive) is defined.
fn index(gpa: Allocator, io: std.Io, path: []const u8, loc_of: *LocOf) !void {
    const bytes = try std.Io.Dir.cwd().readFileAlloc(io, path, gpa, .unlimited);
    const archive = std.fs.path.basename(path);
    if (try elf.archiveMembers(gpa, bytes)) |members| {
        for (members) |m| {
            if (m.file_name.len == 0) continue;
            try indexObject(gpa, m.body, archive, std.fs.path.basename(m.file_name), loc_of);
        }
    } else {
        try indexObject(gpa, bytes, archive, archive, loc_of);
    }
}

fn indexObject(gpa: Allocator, bytes: []const u8, archive: []const u8, file: []const u8, loc_of: *LocOf) !void {
    ...
    for (try obj.symbols(gpa)) |sym| {
        ...
        const gop = try loc_of.getOrPut(gpa, sym.name);
...
fn bump(gpa: Allocator, stats: *Stats, prefix: []const u8, what: []const u8) !void {
    const gop = try stats.getOrPut(gpa, try std.mem.concat(gpa, u8, &.{ prefix, what }));
    if (!gop.found_existing) gop.value_ptr.* = 0;
    gop.value_ptr.* += 1;
}

/// Appends the input-section descriptions for the symbols listed in `hot_path`.
fn emit(
    gpa: Allocator,
    io: std.Io,
    hot_path: []const u8,
    ...
    loc_of: *LocOf,
    v8_archive: ?[]const u8,
    v8_syms: *std.StringArrayHashMapUnmanaged(void),
    stats: *Stats,
    out: *std.ArrayList(u8),
) !void {
    var by_file: std.StringArrayHashMapUnmanaged(std.ArrayList([]const u8)) = .empty;
    var seen: std.StringHashMapUnmanaged(void) = .empty;

    const list = try std.Io.Dir.cwd().readFileAlloc(io, hot_path, gpa, .unlimited);
    var names = std.mem.splitScalar(u8, list, '\n');
    while (names.next()) |name| {
        if (name.len == 0) continue;
        const locs = loc_of.getPtr(name) orelse {
            try bump(gpa, stats, prefix, " nomap");
            continue;
        };
        std.mem.sort(Loc, locs.items, {}, Loc.lessThan);
        for (locs.items) |loc| {
            if (!std.mem.startsWith(u8, loc.section, prefix)) continue;
            if (v8_archive != null and std.mem.eql(u8, loc.archive, v8_archive.?)) {
                ...
                if (std.mem.startsWith(u8, name, "Builtins_")) {
                    try bump(gpa, stats, prefix, " v8-blob-skip");
                    continue;
                }
                try v8_syms.put(gpa, name, {});
                try bump(gpa, stats, prefix, " v8");
                continue;
            }
            if (std.mem.eql(u8, loc.section, prefix) or std.mem.endsWith(u8, loc.section, ".")) {
                try bump(gpa, stats, prefix, " generic");
                continue;
            }
            if (std.mem.indexOfScalar(u8, loc.section, '"') != null or std.mem.indexOfScalar(u8, loc.file, '"') != null) {
                try bump(gpa, stats, prefix, " quote-skip");
                continue;
            }
            const key = try std.mem.concat(gpa, u8, &.{ loc.file, "\x00", loc.section });
            ...
            try file.value_ptr.append(gpa, loc.section);
        }
    }
    try stats.put(gpa, try std.mem.concat(gpa, u8, &.{ prefix, " sections" }), seen.count());
    try stats.put(gpa, try std.mem.concat(gpa, u8, &.{ prefix, " files" }), by_file.count());
